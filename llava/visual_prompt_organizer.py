...
import sys
import os
from llava.visual_prompt_generator import image_blending, color_pool, words_shape,image_blending_fitMed
...
def create_question_prompt_slake(line,shape_color_info,pwise=False,label_aug=False,false_prob=0.7):
    # shape_color_info = [['red', (255, 0, 0), 'rectangle']]
    phrase_list=[]
    for shape_color in shape_color_info:
        
        phrase=f"{shape_color[0]}-colored {shape_color[2]}"
        
        if label_aug and len(shape_color)>3 and shape_color[3] is not None:
            if random.choices([True, False], weights=[1 - false_prob, false_prob])[0]:
                # print(f'Augmented label: {shape_color[3]}')
                phrase+=f' (possibly {shape_color[3]})' 
    ...
    if len(shape_color_info)!=0:
        if pwise:
            question_prompt=f'<image>\nPlease focus on the areas indicated in {phrase_str}. These regions might contain relevant information for answering the question. Use your judgment to determine their significance. Question: {question}'
        else:
            question_prompt=f'<image>\nPlease pay attention to the areas in {phrase_str}. It/They may contain useful information for answering the question. Question: {question}'
...
visual_prompt_config = dict(
    ...
    Slake=[['rectangle',"ellipse","scribble"],'']   
) 
...
def vip_processor(source, image, image_size_anchor, data_args):
    ...
    elif data_args.force_shape is not None:
        visual_prompt_shape_choices, visual_prompt_style  = [data_args.force_shape], ''
        # print(f'Shape forced to {data_args.force_shape}')
    else:
    ...
    if dataset_type in {'vcr'}:
    ...
    elif dataset_type in {'Slake'}:
        if len(source['bboxes']) >14:
            # random choose 14 bboxes, make sure they correspond
            random_indexes= random.sample(range(len(source['bboxes'])), 14)
            source['bboxes'] = [source['bboxes'][i] for i in random_indexes]
            source['entity'] = [source['entity'][i] for i in random_indexes]
        source['segmentations'] = [None] * len(source['bboxes'])
        used_colors = [] 
        ...
        color_rgb = None
        shape_color_info = [] 
        for instance_idx, (bbox, segmentation,entity) in enumerate(zip(source['bboxes'], source['segmentations'],source['entities'])):
            ...
            shape_color_info.append([color_name, color_rgb, predefined_shapes[instance_idx],entity])
        # conversation=source['conversations']
        conversation=create_question_prompt_slake(source,shape_color_info,pwise=data_args.pwise,label_aug=data_args.label_aug)
    ...
    alpha = data_args.alpha if hasattr(data_args, 'alpha') else None
    image_size_anchor=75
    for instance_idx, (bbox, segmentation) in enumerate(zip(source['bboxes'], source['segmentations'])):
        color_name, color_rgb, sampled_shape,_ = shape_color_info[instance_idx] # random.choice(visual_prompt_shape_choices)
        
        bbox= convert_bbox_to_corners(bbox)
        image = image_blending_fitMed(image,  shape = sampled_shape, image_size_anchor = image_size_anchor, rgb_value=color_rgb, bbox_coord= bbox, segmentation=segmentation, visual_prompt_style = visual_prompt_style, alpha = alpha,bboxPos_aug=data_args.bboxPos_aug)
    
    # from matplotlib import pyplot as plt
    # # image.save(f"/home/kangyuzhu/annotations+scripts+outputs/annotations+scripts/ViP-LLaVA/finetune/example_vp/{source['id']}.png")
    # image_root=f"/home/kangyuzhu/annotations+scripts+outputs/annotations+scripts/LLaVA-Med-1.0/outputs/demo_images/{source['id']}"
    # os.makedirs(image_root, exist_ok=True)
    
    # image.save(f"{image_root}/{source['id']}.png")
    # # save the source as a json item:
    # with open(f"{image_root}/{source['id']}.json", 'w') as f:
    #     json.dump(source, f)
    # print(f"Image saved at {image_root}/{source['id']}.png")
